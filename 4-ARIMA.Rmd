...
시계열 그래프는 그림 \@ref(fig:elec-1)에서 볼 수 있다.

```{r elec-1, fig.cap="`elec` 자료의 시계열 그래프"}
autoplot(elec) + 
  labs(x = NULL, y = NULL)
```
...
변환된 자료의 시계열 그래프를 그림 \@ref(fig:elec-2)에 작성해 보자. 
...
그림 \@ref(fig:elec-2)에서 볼 수 있듯이 Box-Cox 변환된 자료는 전체적으로 일정한 변동 폭을 유지하고 있음을 알 수 있다.
그러나 변환된 자료인 $y_{t}^{0.2654}$ 에 대한 의미가 명확하지 않아서 해석에 문제가 있을 수 있다.
비교 차원에서 로그 변환을 실시해 보고, 결과를 비교해 보자.

```{r elec-3, fig.cap="`elec` 자료에 대한 분산안정화 변환의 비교", fig.width=8, fig.height=4}
library(patchwork)
...
두 변환 결과에는 큰 차이가 없다는 것을 그림 \@ref(fig:elec-3)에서 확인 할 수 있다.
...
```{r goog200-1, fig.cap="`goog200` 자료의 시계열 그래프와 ACF", fig.width=8, fig.height=4}
p1 <- autoplot(goog200) + 
...
```{r goog200-2, fig.cap="`goog200`의 1차 차분된 자료의 시계열 그래프와 ACF", fig.width=8, fig.height=4}
...
그림 \@ref(fig:elec-1)에서 살펴본 `elec`는 증가하는 추세와 뚜렷한 계절 성분이 있으며, 
분산이 시간이 흐름에 따라 증가하는 자료이다.
정상성을 만족시키기 위한 변환 절차에서 분산 안정화는 항상 가장 먼저 시행해야 한다.
로그 변환을 시행해서 분산을 안정화 시킨 자료의 시계열 그래프와 ACF는 그림 \@ref(fig:elec-d1)에서 볼 수 있다. 

```{r elec-d1, fig.cap="`elec`에 로그 변환한 자료의 시계열 그래프와 ACF", fig.width=8, fig.height=4}
ln_elec <- log(elec)
p1 <- autoplot(ln_elec) + 
...
함수 `nsdiffs()`\index{R 함수!nsdiffs()}는 seasonal strength를 측정하는 통계량을 근거로 하여 차분 횟수를 추정하는 것이 디폴트이다.
...
작성된 그래프는 그림 \@ref(fig:elec-d2)에서 확인할 수 있다. 

```{r elec-d2,  fig.cap="로그 변환된`elec`에 계절차분을 실시한 자료의 시계열 그래프와 ACF", fig.width=8, fig.height=4}
ln_elec_m <- log(elec) %>% 
  diff(lag = 12)
...
그림 \@ref(fig:elec-d2)에서 볼 수 있듯이 추세 성분이 아직 남아 있는 것으로 보인다.
이제 계절 차분된 자료에 1차 차분을 더 실시한 결과를 살펴보자.
그림 \@ref(fig:elec-d3)에 나타난 패턴을 보면, 비정상 요소가 모두 제거되었음을 알 수 있다.

```{r elec-d3,  fig.cap="로그 변환된 `elec`에 계절차분과 1차 차분을 실시한 자료의 시계열 그래프와 ACF", fig.width=8, fig.height=4}
...
이어서 시계열 그래프도 그림 \@ref(fig:gas-plot)에 작성해 보자.

```{r gas-plot, fig.cap="`gas` 자료의 시계열 그래프"}
...
필요한 그래프는 그림 \@ref(fig:train-r-1)에서 볼 수 있다. 

```{r train-r-1, fig.cap="자료 `train_r`의 정상성 여부 확인을 위한 그래프"}
ggtsdisplay(train_r)
```

그림 \@ref(fig:train-r-1)의 시계열 그래프에는 명확한 추세가 없는 것으로 보이지만, 표본 ACF가 비교적 천천히 모습이 보인다.
...
예측 결과를 그래프로 나타내 보자.
Test data와 함께 표시하는 것이 비교하기 좋을 것이며, 작성된 그래프는 그림 \@ref(fig:test-r-1)에서 확인할 수 있다. 
...
함수 `stl()`\index{R 함수!stl()}로 자료를 분해하고, 이어서 함수 `forecast::seasadj()`\index{R 함수!seasadj()}를 적용해서 계절 요소를 제거해 보자.
...
원자료와 계절 조정된 자료의 시계열 그래프를 그림 \@ref(fig:equip-1)에서 비교해 보자.
...
정상성 판단을 위한 그래프를 그림 \@ref(fig:equip-acf-1)에 작성해 보자.

```{r equip-acf-1, fig.cap="`train_eq`의 정상성 여부 확인을 위한 그래프"}
...
차분된 자료를 대상으로 시계열 그래프와 ACF를 그림 \@ref(fig:equip-acf-2)에 작성해 보자.

```{r equip-acf-2, fig.cap="`train_eq`의 차분한 자료의 정상성 여부 확인을 위한 그래프"}
train_eq %>% 
  diff() %>% 
  ggtsdisplay()
```
...
예측 결과를 그림 \@ref(fig:equip-2)에 그래프로 나타내 보자.
...
그림 \@ref(fig:depart-1)의 시계열 그래프에서 뚜렷한 증가 추세와 계절 요소가 있음을 확인할 수 있다.
또한 계절 요소의 변동 폭이 증가 추세에 따라 함께 증가하고 있는 것도 볼 수 있다.

```{r depart-1, fig.cap="국내 백화점 매출액 자료"}
autoplot(depart.ts) + 
  ylab(NULL)
```

따라서 백화점 매출액 자료는 분산이 동일하지 않고, 추세와 계절 요소가 모두 있는 비정상 시계열자료임을 알 수 있다.
우선 로그변환을 실시한 자료의 시계열 그래프를 그림 \@ref(fig:depart-2)에 작성해서 분산이 일정하게 되었는지 확인해 보자. 
...
이제 시계열 그래프와 표본 ACF의 형태를 그림 \@ref(fig:depart-2)에서 확인해 보자. 
...
그림 \@ref(fig:depart-4)에서 표본 ACF가 1시차에서 6시차까지 매우 천천히 감소하는 것을 볼 수 있고, 
...
더 이상 비정상성 요소가 남아 있지 않다는 것을 그림 \@ref(fig:depart-5)에서 확인할 수 있다.
...
로그 변환과 계절 차분 및 1차 차분으로 비장상성을 모두 제거한 후에는 변환된 자료에 가장 적합한 모형을 식별해야 한다. 그림 \@ref(fig:depart-5)에서 볼 수 있는 표본 ACF와 PACF를 근거로 모형을 식별해 보자.
...
예측 결과를 그림 \@ref(fig:depart-6)에 그래프로 나타내보자. 
예측 결과는 정상성 확보 과정에서 이루어진 변환의 역변환을 실시해서 얻어진 것이다. 

```{r depart-6, fig.cap="백화점 자료 예측 결과"}
forecast(fit_d) %>% 
  autoplot() + ylab(NULL)
```
...
Training data에 대한 시계열 그래프를 그림 \@ref(fig:tour-1)에 작성해 보자.

```{r, tour-1, fig.cap="국내 입국 관광객 수 자료"}
autoplot(train_K) +
  ylab(NULL)
```
...
$\hat{\lambda}=0.09$ 로 추정되었는데, 이 결과를 그대로 적용해서 $y_{t}^{0.09}$ 로 변환시키는 것보다는 변환의 해석이 가능하면서 추정된 $\lambda$ 값과 큰 차이가 없는 $\hat{\lambda}=0$ 에 해당하는 로그 변환을 선택하는 것이 더 좋을 듯 하다. 

로그 변환된 자료에 대한 차분 차수를 결정해 보자. 
먼저 로그 변환된 시계열자료의 시계열 그래프와 ACF를 그림 \@ref(fig:tour-2)에 작성해 보자. 
...
그림 \@ref(fig:tour-3)의 시계열 그래프에서 추세 성분이 남아 있는 것을 볼 수 있고,
...
그림 \@ref(fig:tour-4)의 그래프에서 비정상 요소가 모두 사라진 것을 알 수 있다. 
...
로그변환과 차분을 통해 정산성을 만족한 자료의 ACF와 PACF인 그림 \@ref(fig:tour-4)를 이용하여 모형 식별을 시도해 보자.
...
ARIMA 모형과 `r fit_K_ets2$method` 모형의 예측 결과를 그림 \@ref(fig:tour-5)의 그래프로 비교해 보자. 

```{r tour-5, fig.cap="국내 입국 관광객 수 자료에 대한 예측 결과", fig.width=8, fig.height=8}
library(patchwork)
