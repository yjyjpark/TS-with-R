...
1996년 이후 연간 원유 생산량 자료의 시계열 그래프를 그림 \@ref(fig:oil-1996)에 작성해 보자.
...
Level에 대한 평활상수가 $\alpha =$ `r summary(ses(oil_1996, h = 3))$model$par[[1]] |> round(4)`으로 추정되었다는 것은 level에 큰 변화가 있는 자료를 의미한다. 이제 예측 결과를 그림 \@ref(fig:oil-1996-fore)의 그래프로 나타내 보자.
...
먼저 시계열 그래프를 그림 \@ref(fig:aus-air)에 작성해 보자.
상승 추세가 있는 것을 확인할 수 있다. 

```{r aus-air, fig.cap="1970년부터 2016년까지 호주의 연간 항공기 승객 수"}
autoplot(ausair) +
  labs(title = "Air Transport Passengers Australia", 
       y = NULL)
```

Holt's linear trend 모형을 함수 `holt()`를 사용해서 적합시키고, 15 시차에 대한 예측 결과를 그림 \@ref(fig:aus-air-fore1)에 나타내 보자.
...
Damped Holt's trend 모형에 의한 예측 결과도 그래프로 나타내 보자. 결과는 그림 \@ref(fig:aus-air-fore2)에서 볼 수 있다.
...
그림 \@ref(fig:aus-air-fore1)에서 볼 수 있듯이 Holt's linear trend method에 의한 예측 결과는 지속적으로 증가하고 있으며, 반면에 damped Holt's trend method에 의한 예측 결과는 상승 기울기가 점점 줄어들고 있다는 것을 그림 \@ref(fig:aus-air-fore2)에서 볼 수 있다.

-  Holt-Winters' seasonal method 예제: `fpp2::austourists`

`austourists`는 199년부터 2015년까지 분기별 호주에 입국한 외국인 관광객 수 자료이다. 시계열 그래프는 그림 \@ref(fig:austour-1)에서 볼 수 있다.

```{r austour-1, fig.cap="199년부터 2015년까지 분기별 호주에 입국한 외국인 관광객 수"}
autoplot(austourists) +
  labs(y = NULL, title = "International Tourists to Australia")
```

Holt-Winters' seasonal 모형을 이용해서 예측을 실시해 보자. 먼저 가법 모형으로 예측을 실시하고 결과를 그림 \@ref(fig:austour-2)에 그래프로 나타내 보자.
...
이번에는 승법 모형으로 예측을 실시하고 결과를 그림 \@ref(fig:austour-3)에 그래프로 나타내 보자.
...
`train_air`와 `test_air`의 시계열 그래프는 그림 \@ref(fig:ausair-plot)과 같다. `test_air`는 빨간 선으로 나타냈다.

```{r ausair-plot, fig.cap="`ausair` 자료의 시계열그래프"}
...
ETS 모형의 각 요소에 대한 추정 결과를 그림 \@ref(fig:fit-air-comp)의 그래프로 나타내 보자.
...
```{r fit-air-comp, fig.cap="ETS 모형의 각 요소에 대한 추정 결과 그래프"}
autoplot(fit_air)  
```
...
작성 결과는 그림 \@ref(fig:aus-ETS-fore)에서 볼 수 있다. 

```{r aus-ETS-fore, fig.cap="`fpp2::ausair` 자료에 대한 ETS 모형의 예측 결과",fig.width=8, fig.height=4}
library(patchwork)
p1 <- autoplot(fc_air) + 
...
두 모형의 예측 결과를 그림 \@ref(fig:tour-ets-fore1)의 그래프로 비교해 보자.
...
```{r tour-ets-fore1, fig.cap="`austourists` 자료에 대한 예측 결과 비교", fig.width=8, fig.height=4}
library(patchwork)
...
모형 `r fit_lntour$method`인 `fit_lntour`의 test data에 대한 예측 결과를 예측 구간과 함께 그림 \@ref(fig:tour-ets-fore3)의 그래프로 나타내 보자.

```{r tour-ets-fore3, fig.cap="`austourists` 자료에 대한 예측 결과", fig.width=8, fig.height=4, message=FALSE}
library(patchwork)
...
MASE 값이 지나치게 큰 값이라는 것을 알 수 있다. 예측 결과를 그림 \@ref(fig:dole-fore1)의 그래프로 나타내 보자.
...
이제 예측 결과를 그림 \@ref(fig:dole-fore-2)의 그래프로 나타내 보자.

```{r dole-fore-2, fig.cap="`dole` 자료에 대한 예측 결과", fig.width=8, fig.height=4}
library(patchwork)
p1 <- autoplot(fc_d_1) + 
...
자료의 시계열그래프는 그림 \@ref(fig:hynd-plot)에서 볼 수 있다. 

```{r hynd-plot, fig.cap="`hyndsight` 자료의 시계열그래프"}
...
예측 결과를 그림 \@ref(fig:hyn-fore)의 그래프와 같이 작성해 보자.
