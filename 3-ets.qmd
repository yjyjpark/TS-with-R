...
2. `Seoul_temp.csv`와 `Suwon_temp.csv`는 각각 서울시와 수원시의 월별 평균 기온 자료이며,
...
3. `Baek_pm10.csv`와 `Seoul_pm10.csv`, 그리고 `Suwon_pm10.csv`는 백령도와 서울, 그리고 수원에서 각각 측정된 미세먼지 농도 자료이며,
웹 서버 `https://raw.githubusercontent.com/yjyjpark/TS-with-R/main/Data/` 에서 불러올 수 있다.
