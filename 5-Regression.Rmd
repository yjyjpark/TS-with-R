...
그림 \@ref(fig:ausbeer-1)은 전체 자료의 시계열 그래프이다. Test data는 빨간 색으로 구분했다.
...
```{r ausbeer-2, fig.cap="`ausbeer` 자료에 대한 예측 결과", fig.width=8, fig.height=4}
library(patchwork)
...
그림 \@ref(fig:global-1)은 1970년부터 자료의 시계열 그래프이다. Test data는 빨간 색으로 구분했다.

```{r global-1, fig.cap="자료 `global.txt`의 training data와 test data의 시계열 그래프"}
autoplot(train_g) + 
...
예측 결과를 test data와 함께 표시한 그래프는 그림 \@ref(fig:global-2)에서 볼 수 있다. 
예측 구간의 폭을 비교할 수 있도록 세 그래프의 Y축 구간은 동일하게 설정하였다.  

```{r global-2, fig.cap="자료 `global.txt`에 대한 세 모형의 예측 결과", fig.width=8, fig.height=8}
library(patchwork)
y_lim <- c(-.06, 1.06)
...
전체 기간에 대한 시계열 그래프를 그림 \@ref(fig:AP-1)에 작성해 보자. Test data는 빨간 색으로 구분하였다.

```{r AP-1, fig.cap="자료 `AirPassengers`의 시계열 그래프"}
autoplot(train_AP) + 
...
변환 모수가 $\lambda=-0.31$ 로 추정되었다. 추정된 변환 모수에 의한 변환 결과와 로그 변환에 의한 결과를 비교해 보자. 그림 \@ref(fig:AP-2)에서 볼 수 있듯이 두 변환 결과는 크게 차이가 나지 않는 것으로 보인다. 이런 경우에는 변환 결과에 대한 해석이 가능한 로그 변환을 선택하는 것이 일반적이라고 하겠다.

```{r AP-2, fig.cap="분산 안정화 변환", fig.width=8, fig.height=4}
library(patchwork)
...
그림 \@ref(fig:AP-3)는 세 가지 ETS 모형의 예측 결과를 test data와 함께 나타낸 그래프이다. Box-Cox 변환 자료에 대한 ETS 모형의 예측 결과가 test data와 가장 근접한 것으로 보인다. Test data를 이용해서 모형을 선택하는 것이 바람직한 방식은 아니지만, 모형 비교를 위한 다른 마땅한 방법이 없는 상황을 고려하였다.

```{r AP-3, fig.cap="`AirPassengers` 자료에 대한 ETS 모형의 예측 결과 비교", echo=FALSE, fig.width=8, fig.height=9}
library(patchwork)
...
```{r, AP-4, fig.cap="ARIMA 모형의 예측 결과", echo=FALSE, fig.width=8, fig.height=4}
library(patchwork)
...
```{r AP-5, fig.cap="`AirPassengers` 자료에 대한 예측 결과 비교", echo=FALSE, fig.width=8, fig.height=4}
...
세 변수의 시계열 그래프를 그림 \@ref(fig:elecd-1)에 작성해 보자.
...
이제 두 변수의 관계를 산점도를 이용해서 살펴보자. 그림 \@ref(fig:elecd-2)에서 두 변수 사이에 2차 함수의 관계가 있음을 볼 수 있다. 회귀모형에 변수 `Temp`의 제곱항도 포함시켜야 할 것으로 보인다.

```{r elecd-2, fig.cap="`Demand`와 `Temperature`의 산점도"}
tibble(Demand, Temp) %>% 
...
```{r elecd-3, fig.cap="`Demand`의 예측 결과"}
autoplot(fc)
```
...
`uschange`를 구성하고 있는 다섯 변수의 시계열 그래프를 그림 \@ref(fig:uschange-1)에 작성해 보자.
뚜렷한 추세나 계절 성분이 있는 시계열자료는 없는 것으로 보인다.

```{r uschange-1, fig.cap="`uschange`의 다섯 시계열자료의 그래프"}
...
```{r uschange-2, fig.cap="`uschange`의 다섯 시계열자료의 표본 ACF", fig.width=8, fig.height=5}
ggAcf(uschange)
```

그림 \@ref(fig:uschange-1)과 그림 \@ref(fig:uschange-2)을 근거로 다섯 시계열자료는 모두 정상성을 만족하고 있는 것으로 보인다.
...
```{r uschange-3, fig.cap="`uschange`의 다섯 시계열자료의 산점도 행렬", fig.width=8, fig.height=6}
...
```{r uschange-4, fig.cap="`uschange`의 `Consumption`에 대한 예측 결과 비교", fig.width=8, fig.height=8}
library(patchwork)
y_lim <- c(-1, 2.5)
